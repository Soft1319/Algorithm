...
function [Best_Score,BestFit,Convergence_curve]=Copy_of_SSCSO(SearchAgents_no,Max_iter,lb,ub,dim,fobj)
...
while t<Max_iter
    ...
    RL = 0.35*levy(SearchAgents_no,dim,1.5);
    k=5;
...
%                 Positions(i,j)=exp(z*l) * cos(2*pi*l)*r*(CandidatePosition(j)-rand*pX(i,j));     %%%%%融合螺旋搜索因子后的搜索
    ...
    for i=1:size(Positions,1)
        ...
        %%%%计算每个搜索代理的目标函数
        fitness(i)=fobj(newX);   %%%%计算某个种群个体的适应度值
