...
for func_num = 1:length(F)   
   ...
   %% Run the SSCSO algorithm for "run" times
    ...
    resu = [resu,zz];
    disp(['SS_SCSO：最优值:',num2str(zz(1)),' 标准差:',num2str(zz(2)),' 平均值:',num2str(zz(3)),' 中值:',num2str(zz(4)),' 最差值:',num2str(zz(5))]);
    ...
    for nrun=1:run
        [final,position,iter]=SCSO(pop_size,max_iter,lower_bound,upper_bound,variables_no,fobj);
    ...
    for nrun=1:run
        [final,position,iter]=HHO(pop_size,max_iter,lower_bound,upper_bound,variables_no,fobj);
        final_main(nrun)=final;
        z2(nrun) =  final;
    ...
    for nrun=1:run
        [final,position,iter]=SABO(pop_size,max_iter,lower_bound,upper_bound,variables_no,fobj);
        final_main(nrun)=final;
        z2(nrun) =  final;
    ...
    disp(['SABO：最优值:',num2str(zz(1)),' 标准差:',num2str(zz(2)),' 平均值:',num2str(zz(3)),' 中值:',num2str(zz(4)),' 最差值:',num2str(zz(5))]);
     ...
     for nrun=1:run
        [final,position,iter]=MSCSO(pop_size,max_iter,lower_bound,upper_bound,variables_no,fobj);
        final_main(nrun)=final;
        z2(nrun) =  final;
    ...
    if box_pp == 1 
        ...
        set(gca,'XTickLabel',{'SS_SCSO','DBO','SSA','SCSO','HHO','SABO','MSCSO'});
...
A = cellstr (A);
A = [A,num2cell(RESULT)];
title = {" "," ","SS_SCSO","DBO","SSA","SCSO","HHO","SABO","MSCSO"};
A = [title;A];

writecell(A,'result1.xls')
...
writecell(B,'ranksumresult1.xls')
